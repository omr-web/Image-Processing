...
class Image {
 ...
 public:
  ...
  // LAB0 Fonksiyonları //
  void Adjust_Image();
  Image Change_Dimension(int);
   // LAB1 Fonksiyonları //
   Image _ImageNegatives();
   Image _ImageLog();
   Image _ImageGama(double);
   Image _ImageGama_BiggerThanOne(double);
   Image _Average_Filter (int);
   Image _Median_Filter(int);
   Image thresholdImage(float threshold = 127.0, float lowValue = 0.0, float highValue = 255.0);

   // LAB2 İÇİN FONKSİYONLAR //
   Image HistogramEqualization();
   Image  SpatialFilters(int,int);
   Image  GaussianFilters(int);
   int Find_Different_Pixel_Values();
   // LAB3 İÇİN FONKSİYONLAR //
    Image Create_Image(int , int ); // kullanılmıyor
    void  Fourier_Adim0();
   Image Fourier_Transform();
   Image Print_Fourier();
   Image Inverse_Fourier(); // kullanıılmıyor
   Image InverseFourier2();
   // LAB4 İÇİN FONKSİYONLAR
    Image Sobel_Gradient();
    Image Scale(); // kullanılmıyor
    Image H_uv();
